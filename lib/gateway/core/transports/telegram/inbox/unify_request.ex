defmodule Transports.Telegram.Inbox.UnifyRequest do
  ...
  def call(%{message: %{chat: _chat} = _message, transport: _transport} = params) do
    params
    |> init_request()
  ...
  def get_event_type(%{message: %{edit_date: _edit_date}}), do: "update_inbox"
  ...
  def fill_attachments(request, %{message: %{document: document}}) do
    Inbox.Structs.UnifiedRequest.add_attachment(request, %{
      id: document.file_id,
  ...
  def fill_attachments(request, %{message: %{voice: voice}}) do
    Inbox.Structs.UnifiedRequest.add_attachment(request, %{
      id: voice.file_id,
  ...
  def fill_attachments(request, %{message: %{video: video}}) do
    Inbox.Structs.UnifiedRequest.add_attachment(request, %{
      id: video.file_id,
  ...
  def fill_attachments(request, %{message: %{sticker: sticker}}) do
    Inbox.Structs.UnifiedRequest.add_attachment(request, %{
      id: sticker.file_id,
      width: sticker[:width],
